...
func (g *PropGenerator) outObject(w io.Writer, t *ast.StructType, i int) error {
	...
	for _, f := range t.Fields.List {
		var existPublicField bool
		for _, name := range f.Names {
			if r := name.Name[0]; 'A' <= r && r <= 'Z' {
				existPublicField = true
				break
			}
		}
		if !existPublicField {
		...
		if len(f.Names) != 1 {
			return fmt.Errorf("only one field definition can be specified per line: %v", f.Names)
		}
		g.outTab(w, i+1)
